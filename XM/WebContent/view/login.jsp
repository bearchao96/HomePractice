...
<div style="text-align: center ;margin-top: 1%">
    <font style="font-size: 50px">账号登录</font>
</div>

<div style="width: 50%; background-color: limegreen; text-align: center; margin-left: 25%; margin-top: 40px; padding-top: 5%; padding-bottom: 5%;margin-bottom: 5%">
    <h4>${ LoginMSG01 }</h4>
    <div style="width: 55%; text-align: center; margin-left: 20%;">
        <form class="form-horizontal" action="/XM/indexServlet?method=Login"  method="post">
            <div class="form-group" id="div001">
                <label for="inputEmail3" class="col-sm-2 control-label" style="color: black">账号</label>
                <div class="col-sm-10">
                    <input type="text" class="form-control" id="input001" name="u_usernum"
                           placeholder="请输入账号" onblur="isUsers()">
                </div>
            </div>
            <div class="form-group" id="div002">
                <label for="inputPassword3" class="col-sm-2 control-label" style="color: black">密码</label>
                <div class="col-sm-10">
            ...
            <div class="form-group">
                <div class="col-sm-offset-2 col-sm-10">
                    <div class="checkbox">
                        <label> <input type="checkbox" style="color: black"> 记住密码
                        </label>
                    </div>
                </div>
            </div>
...
<script type="text/javascript">
    ...
    function isUsers(){
        ...
        if(usernum === ""){
            ...
            $("#but001").attr("disabled",true);
        }else if(password === ""){
            //提示用户密码不能为空
            $("#input002").attr("placeholder","密码不能为空")
            $("#div002").addClass("has-error");
            $("#but01").attr("disabled",true);
        }else{
            //进行登录
            //解除禁止登录
            $("#but01").attr("disabled",false);
            $("#div002").addClass("has-success");
            $(document).ready(function(){

                //AJAX 的post请求
                $.post("/XM/indexServlet?method=isLogin",{name:usernum},function(data){
                    //判断
                    if(data === "false"){
                        //用户不存在
                        $("#input001").val("")
                        $("#input001").attr("placeholder","该用户名不存在")
                        $("#div001").addClass("has-error");
                        $("#but01").attr("disabled",true);
                    }else{
                        $("#div001").removeClass("has-error");
                        $("#div001").addClass("has-success");
                        $("#div002").removeClass("has-error");
                        $("#div002").addClass("has-success");
                        $("#but001").attr("disabled",false);
                    }
                })
            })
    ...
    $("#button01").click(function(){
        window.location.href = "/XM/view/index.jsp"
    })

    /*如果是注册跳转就返回个用户账号 让其自行登录*/
    $.post("/XM/indexServlet?method=findByNum",function (data) {
        if(data != ""){
            alert("您已注册成功，账号为"+data+"请登录")
        }
